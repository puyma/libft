...
/*   Updated: 2022/09/11 15:06:53 by mpuig-ma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_line(const char *buf, char **remains)
{
	char	*line;
	char	*substr;

	substr = ft_substr(buf, 0, ft_strchr(buf, '\n') - buf);
	if (*remains == NULL)
		line = substr;
	else
	{
	...
	*remains = ft_substr(buf, ft_strchr(buf, '\n') - buf + 1, ft_strlen(buf));
	return (line);
}
